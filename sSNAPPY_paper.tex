...
The first step, performed internally by \emph{sSNAPPY}, is to estimate sample-specific log fold-change (\(\delta_{gi} = \mu_{gi} - \mu_{gi'}\)) across all genes \(g\) within each treated sample \(i\), by subtracting expression estimates for the corresponding baseline samples \(\mu_{gi'}\) from those in the treated groups.
\emph{sSNAPPY} supports any number of treatment levels and the numbers of samples in each treatment group do not have to be identical.
...
We then use these weighted estimates of logFC (\(\delta_{gi}^* = w_g\delta_{gi}\)) in the calculation of all pathway perturbation scores.
...
Thus, we define the single-sample perturbation score (\(S_{ip}\)) for a given pathway \(p\) in a treated sample \(i\):

\[
\begin{aligned}
S_{ip} = \sum_{g \in G_p} \lbrack S_{gip} - \delta_{gi}^*\rbrack \text{, where} \\
S_{gip} = \delta_{gi}^* + \sum_{g' \in U_{gp}} \beta_{gg'p} \frac{S_{g'ip}}{N_{g'p}} 
\end{aligned}
\]
where:
...
\item
  \(S_{gip}\) is the gene- and sample-specific perturbation score for pathway \(p\)
\item
  \(\delta_{gi}^* = w_g\delta_{gi}\) is the weighted logFC of gene \(g\) in sample \(i\) as described above
\item
...
\item
  \(S_{ip}\) is the accumulated pathway perturbation score for pathway \(p\) in sample \(i\)
\end{itemize}
...
To scale the single-sample pathway perturbation scores (\(S_{ip}\)) so they are comparable across pathways and to test for significance of individual scores, null distributions of perturbation scores for each pathway are generated through a sample permutation strategy, retaining the correct correlation structure between genes within a pathway.
We recommend users to perform a minimum of 1000 permutations, requiring at least 8 unique samples.
Subsequently, the median and median absolute deviation (MAD) of the permuted perturbation scores will be calculated and used to normalise the raw perturbation scores to robust \(Z\)-scores (\(Z_{ip} = \frac{S_{ip} - median_{n \in N, i \in I}({S_{i^np}})}{MAD_{n \in N, i \in I}({S_{i^np}})}\)) and obtain associated two-sided \emph{p}-values.
With each permutation, for example the \(n\)-th round, column names (i.e.~sample labels) for the expression matrix are randomly shuffled while the rest of the scoring algorithm remains unchanged, resulting in one permuted score being computed for each pathway \(p\) in each permuted sample \(i^n\).
Performing \(N\) rounds of permutations on a dataset where the total number of treated sample equals \(I\), \(N \times I\) permutated scores will be generated for each pathway \(p\).
...
The output of \texttt{weight\_ss\_fc()} is a \texttt{list} where one element is a matrix of weighted single-sample logFCs (\(\delta_{gi}^*\)), with rows corresponding to genes and columns to samples, and the other element is the vector of gene-wise weights (\(w_g\)) used to calculate the weighted logFC (\(\delta_{gi}^*\)), as described above.

The matrix of \(\delta_{gi}^*\) values are then passed to pathway topologies to compute the gene-wise perturbation scores for all genes included in a pathway, before being summed into a single score for each pathway.
The string \texttt{ENTREZID:} was also added to all row names of the \(\delta_{gi}^*\) matrix to be compatible with the format pathway topologies were retrieved in.
...
The pathway-level perturbation scores (\(S_{ip}\)) are returned as a data.frame containing sample and gene-set names.
In the following steps, we first calculate the gene-level contributions to each pathway (\(S_{gip}\)) using the function \texttt{raw\_gene\_pert()} and then obtain pathway-level summaries using \texttt{pathway\_pert()}.
...
Since sample labels will be permuted randomly, sample metadata is not required by the \texttt{generate\_permuted\ \_scores()} function, instead, users only need to specify the number of treatment groups in the study, including the control level.
...
11 out of the 315 tested KEGG pathways passed the selection threshold (Table \ref{tab:Table1}).
...
Among the stress-related marker genes reported, 6 of them(\emph{CXCR4}, \emph{FOS}, \emph{GADD45B}, \emph{IL6}, \emph{JUN}, and \emph{TNF}) were implicated in pathways that were considered to be significantly impacted by sSNAPPY.
...
A key gene involved in those pathways was \emph{CHEK1}, which was also found in significantly perturbed pathways detected by sSNAPPY: the p53 signaling pathway, Cellular senescence and the Human T-cell leukaemia virus 1 infection pathway.
