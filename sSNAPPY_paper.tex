...
Subsequently, the median and median absolute deviation (MAD) of the permuted perturbation scores will be calculated and used to normalise the raw perturbation scores to robust \(Z\)-scores, which is defined as:

\[
Z_{hip} = \frac{S_{hip} - median_{n \in N}({S_{h^ni^np}})}{MAD_{n \in N}({S_{h^ni^np}})}
\]
