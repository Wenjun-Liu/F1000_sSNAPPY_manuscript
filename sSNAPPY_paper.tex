...
We also performed pathway analysis on this example dataset using three other methods to compare their performances against \emph{sSNAPPY}.
...
The first method applied was the existing group-level topology-based method \emph{SPIA}\citep{Tarca2009}.
Since \emph{SPIA} only considers pathways containing at least one differentially expressed genes, it has a lower sensitivity and was not able to detect any significant pathway perturbation in this dataset where very few genes were differentially expressed.
Additionally, we also performed pathway analysis on this example dataset using two non-topological-based approaches: 1) \emph{GSEA}\citep{Subramanian2005-lx} basing on the ranking statistics derived from the differential expression analysis and 2) the fast version of rotation gene set testing for linear models (\emph{roast})\citep{wu2010roast} \emph{fry}, which does not rely on DE analysis results.
While there was large overlap in the significantly impacted pathways revealed by \emph{GSEA}, \emph{fry} and \emph{sSNAPPY}, \emph{sSNAPPY} identified processes that could not be detected using the other two methods.\\
Moreover, although both \emph{GSEA} and \emph{fry} try to infer the directionality of change in pathway activity, they only base it on whether genes from a pathway are more enriched in up- or down-regulated genes, ignoring the fact that genes can have both an activator and a repressor role in biological processes.
For example, the pathway \textbf{Terminal pathway of complement} was considered as significantly inhibited by \emph{sSNAPPY} but significant activated by \emph{fry}.
\emph{fry} identified this pathway as activated as the only expressed gene in this pathway \emph{CLU} was significantly up-regulated among post-chemotherapy samples.
However, the topology of this pathway indicates that \emph{CLU} is a suppressor of the process, which \emph{sSNAPPY} was able to account for.
Through comparing \emph{sSNAPPY} against existing pathway analysis methods, both topological and non-topological, the strength and novelty of \emph{sSNAPPY} were demonstrated.
