---
title: "Comparison_other_methods"
output:
...
Genes with a FDR < 0.05 were determined to be differentially expressed. `r nrow(topTable %>% dplyr::filter(DE))` DEGs were detected among the `r nrow(topTable)` tested genes:
...
topTable %>%
    ...
    datatable(
       ...
       caption = htmltools::tags$caption(
            htmltools::em(
                "Significant changes in gene expression induced by chemotherapy in clinical HGSOC samples.")
        )
    ) %>% 
    formatStyle(
        'logFC', 
...
Among the `r nrow(topTable %>% dplyr::filter(DE))` DEGs, `r nrow(topTable %>% dplyr::filter(DE, logFC < 0))` are down-regulated while `r nrow(topTable %>% dplyr::filter(DE, logFC > 0))` are up-regulated.

# Pathway analysis using other methods

## GSEA

To perform the gene-set enrichment analysis (GSEA), ranking statistics of each gene were calculated based on the DE analysis results by `-sign(logFC)*log10(PValue)`.
...
2 pathways: Translocation of ZAP-70 to Immunological synapse and Generation of second messenger molecules were found to be significantly impacted by the chemotherapy by all methods, and 32 pathways were significant in results returned by *sSNAPPY*, *fry* and *GSEA*. The DNA damage response and repair pathway **SUMOylation of DNA damage response and repair proteins** was reported in the original study and considered as significantly impacted by all 3 methods: *sSNAPPY*, *fry* and *GSEA*. However, only *sSNAPPY* is capable of accounting for gene-gene interaction to predict the directionality of change in pathway acitvities. 

```{r}
overlap_3methods <- allMethod_sig[c("GSEA", "fry", "sSNAPPY")] %>%
    Reduce(intersect,.) %>%
    setdiff(., allMethod_sig$SPIA)
str_remove_all(overlap_3methods, "reactome.")
```
