...
<!-- Given your original text, if you really want p-values they should be obtained directly from the permuted distribution, not from the transformation -->
<!-- The methods for this are easily findable using google -->
<!-- Realistically, these median/mad values really only start to converge to their true values N > 2500 -->

Apart from assessing whether a pathway's activity changed significantly within an individual sample, users may also be interested in detecting changes across all samples within a treatment, which can be performed by modelling scores with regression models, incorporating Smyth's moderated $t$-statistic[@Smyth_2004] as implemented in *limma*[@limma_2015].
...
Importantly, to run *sSNAPPY*, the row names of the expression matrix must be specified as EntrezGene IDs, for compatibility with pathway databases. 
...
The factor defining the paired structure is passed to the `weight_ss_fc()` function through the `groupBy` parameter. 
...
<!-- Ideally, we'd rename the second element so all users were clear these values already had weights applied -->
...
In the above example, the string `ENTREZID:` was also added to all row names of the $\delta_{ghi}^*$ matrix to be compatible with the format KEGG pathway topologies were retrieved in. 
...
All possible random pairs between samples will be sampled by default, unless otherwise specified by the `NB` parameter. 
...
<!-- I'm not sure we can claim normality from a boxplot. We can claim symmetry however. -->
<!-- If we want to claim normality, we should use qqplots. See example code below -->
...
<!-- Here's my code for a qqplot -->
```{r stevie-qq, echo = FALSE, eval = FALSE}
set.seed(234)
permutedScore %>%
    keep(~all(.!=0)) %>%
    .[sample(seq_along(.), 6)] %>%
...
## My take from the above is that none of these are showing signs of 'heavy-tails'
```
...
The distributions obtained from label permutations are then used to convert each pathway-level score into a scaled robust $Z$-score using the function `normalise_by_permu()`.
<!-- I think this needs more thought. Permuted p-values may be a better option -->
<!-- There is a paper here (https://academic.oup.com/bioinformatics/article/25/12/i161/189924) which is partially relevant, but we won't have enough samples for this to apply. -->
<!-- The use of permuted p-values is still worth pondering though -->
...
In order to minimise spurious results, Smyth's moderated t-statistics[@Smyth_2004] are able to be applied across the complete dataset, and given that we are using Z-scores, a constant variance was assumed across all pathways.
To perform this analysis, the robust Z-scores were converted to a matrix, and the standard *limma* methodologies used.
...
<!-- Font sizes are still too small in Figure 4. I've had a quick play and I think they more closely match the text in the caption -->
<!-- I've had so many papers come back with this kind of issue -->

```{r Figure4, out.width='100%', fig.width = 12, fig.height = 5, fig.cap = "Significantly perturbed KEGG pathways identified among post-chemotherapy samples using sSNAPPY, colored by (A) pathways’ predicted direction of changes and (B) pathways’ -log10(p-values). Pathways with a FDR < 0.05 in the moderated t-test were included."}
set.seed(123)
