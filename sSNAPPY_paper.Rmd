...
abstract: 
 ...
 Not only were we able to predict the direction of significant perturbation of pathways discussed in the original study, but *sSNAPPY* was also able to detect significant changes in other biological processes, yielding far greater insight into the response to treatment.
 *sSNAPPY* represents a novel pathway analysis strategy that takes pathway topology into consideration to predict impacted biological pathways, both within related samples and across treatment groups. 
...
This dataset consists of 11 high-grade serous ovarian cancer (HGSOC) patient samples taken before and after chemotherapy[@Zhang2022].
...
```{r Figure3, out.width='100%', fig.width=12, fig.height=12, fig.cap="(A) Q-Q plot and (B) distributions of permuted perturbation scores of six randomly selected pathways. All sampled empirical distributions are approximately normally distributed with a mean of zero.", fig.pos= "p"}
set.seed(234)
...
`r nrow(sigPathway)` out of the `r nrow(top_table)` tested Reactome pathways passed this selection threshold (Table \@ref(tab:Table1)).
...
The representative pathways enriched in the stress-associated cluster were *IL6-mediated signaling events*, *TNF signaling pathway*, and *cellular responses to stress*, while the other post-chemotherapy cell dominated cluster was enriched for the *Cell cycle*, *DNA repair*, *Homology directed repair (HDR) through homologous recombination*, and *Fanconi anaemia pathway*.
*sSNAPPY* not only detected the significant perturbation of many of the pathways reported in the original study (*Cell Cycle*, *DNA Repair* and *Homology Directed Repair*) but also predicted that those pathways were inhibited by chemotherapy, supporting the known tumour inhibition effect of the treatment (Table \@ref(tab:Table1)).

```{r, echo=FALSE, eval=FALSE}
# Code for generating table 2 (NOT USED ANYMORE)
stress_markers <- c("JUN", "FOS", "IL6", "TNF", "CXCR4", "SNAI1", "VIM", "GADD45B", "MCL1")
repair_markers <- c("PCN1", "CHEK1", "HMGB2", "BRAC2", "FANCI", "POLD1")
load(system.file("extdata", "entrez2name.rda", package = "sSNAPPY")) 
sapply(gsTopology, rownames) %>%
    do.call(cbind, .) %>%
    as.data.frame() %>%
    pivot_longer(
        cols = everything(),
    ...
    dplyr::filter(
        gs_name %in% sigPathway$gs_name, 
        mapTo %in% c(repair_markers, stress_markers)
    ) %>%
    left_join(sigPathway) %>%
      mutate(
        Direction = ifelse(logFC < 0, "Inhibited", "Activated"), 
        gs_name = str_remove_all(gs_name, "reactome.")
    ) %>%
    dplyr::select(
        Pathway = gs_name, gene_name = mapTo, Direction, FDR = adj.P.Val
                  ) %>%
    chop("gene_name") %>%
    mutate(
    ...
    dplyr::rename(`Marker Gene` = gene_name) %>%
    write_csv(here::here("sSNAPPY_paper_files/Table2.csv"))
...
```{r Table1, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="p"}
knitr::kable(
  table1, format = "latex",
  caption = "Significantly impacted Reactome pathways identified among post-chemotherapy samples using sSNAPPY.", 
  booktabs = TRUE,
...
The `plot_gs_network()` function allows users to easily convert a list of relevant biological pathways to a network where the edges between pathway nodes represent overlapping genes. 
...
```{r Figure4, out.width='100%', fig.width = 14, fig.height = 7, fig.cap = "Significantly perturbed Reactome pathways identified among post-chemotherapy samples using sSNAPPY, colored by (A) predicted direction of changes and (B) -log10(p-values). Only the top 15 most highly ranked are shown.", fig.pos="hp", message=FALSE}
set.seed(456)
...
The most recent categories for both KEGG and Reactome databases were curated from their respective websites ([KEGG website](https://www.genome.jp/kegg/pathway.html) & [Reactome website](https://reactome.org/PathwayBrowser/#/R-HSA-1632852&PATH=R-HSA-9612973)) and included as parts of sSNAPPY.
...
```{r Figure5, out.width='100%', fig.width = 13, fig.height = 11,fig.cap = 'Significantly perturbed Reactome pathways identified among post-chemotherapy samples using sSNAPPY, colored by community structures detected through the louvain algorithm. The main biological processes associated with the top 15 pathways that were most perturbed by chemotherapy are shown.', fig.pos="hp",  warning=FALSE}
...
```{r Figure6, out.width='100%', fig.width = 12, fig.height = 9, fig.cap = 'Significantly perturbed Reactome pathways identified among post-chemotherapy samples using sSNAPPY, showing any genes in the top 200 ranked by magnitude of change in expression, and the pathways they are likely contributing to. Only the 15 most significantly perturbed pathways are shown.', fig.pos="h"}
set.seed(123)
plot_gs2gene(
    normalisedScores = sigPathway, 
    gsTopology = gsTopology, 
...
# To check that the retrieved topology is in the right format (rows: downstream; cols: upstream)
# find genes that are at the bottom of the topoplogy of SUMOylation of DNA replication proteins
noDownstream <- rownames(gsTopology$`reactome.SUMOylation of DNA replication proteins`)[apply(gsTopology$`reactome.SUMOylation of DNA replication proteins`, 1, function(x){
    sum(x != 0) == 1
})] 
# They are SUMO1, 2, 3, which appears to be the very downstream genes that don't have any further targets
entrez2name %>%
    dplyr::filter(
        entrezid %in% noDownstream
        )
# find all root genes in this pathway
root <- rownames(gsTopology$`reactome.SUMOylation of DNA replication proteins`)[apply(gsTopology$`reactome.SUMOylation of DNA replication proteins`, 2, function(x){
    sum(x != 0) == 1
})] 
# They are TOP2A, PCNA, TOP1, AURKA, TOP2B, all of which are root genes
entrez2name %>%
    dplyr::filter(
        entrezid %in% root
        )
# Take a root gene TOP2A: ENTREZID:7153, its gene-wise scores equal the weighted ssFC
identical(
    genePertScore$`reactome.SUMOylation of DNA replication proteins`["ENTREZID:7153",], 
    weightedFC$weighted_logFC["ENTREZID:7153",])
...
```{r Figure7, out.width='100%', fig.width = 12, fig.height = 7, fig.cap = 'Gene-level perturbation scores for the top 15 genes in the \"SUMOylation of DNA replication proteins\" pathway ranked by average contribution to the perturbation score. Samples were annotated by patient chemotherapy response score (CRS), along with the range for sample-level Z-scores as a guide to sample-specific pathway perturbation. The genes CDCA8, UBE2I, BIRC5 and three SUMO family genes SUMO1, SUMO2, SUMO3 were identified as possible key drivers of the inhibition of this pathway.', fig.pos="hp"}
...
From this heatmap we can identify candidate genes which are likely to be making the biggest contribution to the inhibition of the *SUMOylation of DNA replication proteins pathway* post chemotherapy, such as the three SUMO family genes *SUMO1*, *SUMO2*, *SUMO3*, and genes *CDCA8*, *UBE2I*, and *BIRC5* (Figure \@ref(fig:Figure7)).
SUMOylation is a post-translational modification process that has been implicated in various cancers[@Hu2019].
The expression of *SUMO1* gene is associated with tumour initiation and progression, while *SUMO2* and *SUMO3* primarily modifies stress-related proteins, and their roles in cancer have not been well studied[@Hu2019].
Additionally, the other three candidates *CDCA8*, *UBE2I*, and *BIRC5* have all been associated with tumour progression and invasiveness and studied in the context of ovarian cancer.
...
CRS is an indicator of the relative length of progression-free survival after chemotherapy, where a score of 3 represents the longest survival[@Michaan2018].
...
```{r Figure8, out.width='80%', fig.height=4, fig.width=6, fig.cap = 'Single-sample logFC (ssFC) of potential key genes driving the inhibition of \"SUMOylation of DNA replication proteins\" pathway as a response to chemotherapy in HGSOC tumours. The red dash line indicates a ssFC of 0 (i.e. no change in expression).', message=FALSE, fig.pos="h"}
(weightedFC$weighted_logFC / weightedFC$weight) %>%
    ...
    left_join(entrez2name) %>%
    dplyr::filter(mapTo %in% c("CDCA8", "UBE2I", "BIRC5", "SUMO1", "SUMO2", "SUMO3")) %>%
    ggplot(
...
The dataset analysed in this manuscript is stored in the data directory of [this GitHub repository](https://github.com/Wenjun-Liu/sSNAPPY). 
