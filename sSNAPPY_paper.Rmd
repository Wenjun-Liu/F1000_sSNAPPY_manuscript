...
abstract: 
 ...
 Whilst many strategies test for over-representation within pre-defined gene-sets for genes showing changed expression[@Subramanian2005-lx; @Young2010-jw], these analyses typically do not account for gene-gene interactions encoded by pathway topologies, and are not able to directly predict the directional change of pathway activity. 
...
<!-- Are we meant to exclude references from the Abstract? (Had to comment down here, sorry...)-->
<!---I was also trying to avoid using 'common' near the start of the the first two sentences-->
...
<!-- I wonder if we need to demonstrate that SPIA cant find the same results as sSNAPPY? -->
...
This article defines how *sSNAPPY* computes changes in gene expression within paired samples, and propagates this through gene-set topologies to predict the perturbation in pathway activities within paired samples, before providing summarised results across an entire dataset which would include more robust levels of biological replication (Figure \@ref(fig:Figure1)). 
...
<!-- I'm also wondering if we change logFC to 'fold-change' or 'effects'. Reality is the logFC is just log-transformed (i.e. a convenient mathematical transformation of) fold-change -->
...
* $\delta_{ghi}^* = w_g\delta_{ghi}$ is the weighted logFC of gene $g$ in treatment $h$ as described above
...
With each permutation, column names (i.e. sample labels) for the expression matrix are randomly shuffled while the rest of the scoring algorithm remains unchanged.
...
<!-- The above line needs to be a better more formal equation, using $Z_{hip}$ & $S_{hip}$. Perhaps you should even define the set of permuted values formally. -->
...
<!-- The above numbers produce 10098 genes whilst the earlier text says 11101. Is this a problem? -->
...
<!-- I don't understand this. The table has 11 pathways. Did you use a different threshold? -->
...
The returned plot is a ggplot2 [@Wickham2009] object, meaning that components of the plotting theme and other parameters can be customized as for any other ggplot objects.
...
<!-- The size of the legends & their text needs to be increased to match the rest of the figure -->
<!-- Why not just make the figure smaller? 18 x 10 seems very large -->
```{r Figure4, out.width='100%', fig.width = 18, fig.height = 10, fig.cap = "Significantly perturbed KEGG pathways identified among post-chemotherapy samples using sSNAPPY, colored by (A) pathways’ predicted direction of changes and (B) pathways’ -log10(p-values). Pathways with a FDR < 0.05 in the moderated t-test were included."}
set.seed(123)
# Plot the network structure
p1 <- plot_gs_network(
  normalisedScores = sigPathway, gsTopology = gsTopology, 
  colorBy = "status", gsNameSize = 8, 
  nodeSizeScale = c(8, 12)
) +
...
<!-- Isn't the Louvain method a layout? Or is it a strategy for community detection? -->
<!-- Again, setting these figures to be so large leads to legends which are unreadable. -->
<!-- This does need to be rectified, either by setting their size or by shrinking the size of the pdf -->

```{r Figure5, out.width='80%', fig.width = 16, fig.height = 10, fig.cap = 'Significantly perturbed KEGG pathways identified among post-chemotherapy samples using sSNAPPY, colored by community structures detected through the louvain algorithm. The main biological processes perturbed by the chemo-therapy were shown.'}
set.seed(123)
plot_community(
    normalisedScores = sigPathway,
    gsTopology = gsTopology, 
...
<!-- I can see a weird error in the pdf here! -->
...
An alternative strategy may obviously be to select genes based on test-statistics, however, this decision is up to the individual researcher.
...
<!-- Do we need to specify which annotation build this is derived from? -->
...
<!-- Again the legend sizes are too small -->

```{r Figure6, out.width='80%', fig.width = 16, fig.height = 12, fig.cap = 'Significantly perturbed KEGG pathways identified among post-chemotherapy samples using sSNAPPY, showing any genes in the top 500 ranked for differential expression, and which pathways they are likely contributing to. No genes in the top500 were found to contribute to the perturbation of the AMPK signaling pathway.'}
set.seed(123)
plot_gs2gene(
    normalisedScores = sigPathway, 
    gsTopology = gsTopology, 
...
<!-- For the following figure, is that legend centred correctly?  -->
<!-- It looks like zero is in the blue end of the range.  -->
<!-- This can be a a bit of an issue with pheatmap, but is very fixable and fixing this should be done in the function. -->
<!-- And yeah, I'd consider it a bugfix -->
