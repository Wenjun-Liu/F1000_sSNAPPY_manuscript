...
<!-- I thought with n samples, there will be n! types of possible arrangement. Since order matters in this case, it's a permtuation not a combination. 6! only gives 720 possibilities and due to the match-paired design required by the method, 7 samples isn't plausible. So the smallest number required to have more than 1000 permutations is 8. Does that make sense?  -->
...
```{r calc-permuted-scores, eval=FALSE}
...
<!-- I wonder if there's a way to compute similarity based on perturbation scores instead of straight overlap?-->
<!-- Just a crazy idea, but maybe this might push things together based on key inflection points in each pathway? -->
<!-- Might be too much for this stage though -->
...
<!-- And yeah, you're right! You do need to tidy up this function -->
<!-- I don't see any need to hardwire in the column names robustZ or pvalue -->
<!-- Instead, I think it would be better use my match.arg strategy and the column names-->
<!-- I decided to make a pull request for sSNAPPY itself... -->
<!-- I also think switching "gs_name" to "pathway" is also more informative, but I didn't do that -->
...
However, knowing which genes are implicated in the perturbation of pathways, particularly those which influence multiple pathways, can provide valuable insights for researchers for hypotheses generation about underlying biological mechanisms. 
Therefore, sSNAPPY presents another visualisation feature called `plot_gs2gene`, which enables the inclusion of pathway genes in network structures.
Users can provide a vector of fold-changes to colour the genes by their estimated change in expression. 
As pathways are often made of hundreds of genes, we recommend to filter for genes more likely to be playing a significant role.
...
Since pathway topologies were retrieved in Entrez IDs, by default, genes’ Entrez IDs will be used to annotate gene nodes in the plot. 
