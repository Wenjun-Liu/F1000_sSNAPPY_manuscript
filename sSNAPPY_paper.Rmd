...
We recommend users to perform $K \geq 1000$ permutations, requiring N $\geq 8$ samples in total, as minimally represented by $I = 4$ pairs of treated and control samples (i.e. $H = 1$).
<!-- Does this return K*I individual permutation scores? You had this number last time... -->
<!-- I've modified the formula below but I'm really not sure how you want to denote permutations. -->
<!-- I'm obviously a bit far away from this, but I assume nesting and treatment labels are ignored during permutations? -->
Subsequently, the median and median absolute deviation (MAD) of the permuted perturbation scores will be calculated for each pathway, and used to normalise the raw perturbation scores $S_{hip}$ to robust $Z$-scores, $Z_{hip}$.

<!-- Yeah, I'm confused by this formula. Let's leave it out. -->
<!-- The way you had it was to have sample & treatment specific permutations, which I didn't think was correct -->
<!-- $$ -->
<!-- Z_{hip} = \frac{S_{hip} - median_{k \in K}({S_{kp}})}{MAD_{k \in K}({S_{kp}})} -->
<!-- $$ -->
...
<!-- I really don't understand this. How are permutations sample & treatment specific if we've shuffled labels? -->
<!-- Isn't it just a set of numbers which is agnostic to these categories? -->
<!-- Dont we obtain a reference set & subtract these values from all $S_{hip}$ values? -->
<!-- I'm just really confused by the following sentence -->
The permuted pathway perturbation score for pathway $p$ in a sample that is assigned to be of treatment $h$ and patient $i$ in the $n$-th round of permutation is represented as $S_{h^ni^np}$, and $N$ is the total rounds of permutation performed.
...
```{r Figure6, out.width='80%', fig.width = 9.6, fig.height = 8, fig.cap = 'Significantly perturbed KEGG pathways identified among post-chemotherapy samples using sSNAPPY, showing any genes in the top 500 ranked for differential expression, and which pathways they are likely contributing to. No genes in the top500 were found to contribute to the perturbation of the AMPK signaling pathway.'}
set.seed(123)
plot_gs2gene(
    normalisedScores = sigPathway, 
    gsTopology = gsTopology, 
