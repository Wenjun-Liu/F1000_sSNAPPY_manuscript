...
Retrieving pathway topology information from a chosen database is the next key step required during preparation for running *sSNAPPY* needs to be retrieved, and this is the only step requiring internet access.
...
<!-- Did you want to include a comment mentioning your findings about the structure of topology matrices, and which need transposing here? -->
...
<!-- Was this >11, or $\geq$11? -->
We considered a gene detectable if we observed >1.5 counts per million in >11 of the 22 samples, ideally representing all samples from a complete treatment group. 
...
permutedScore <- generate_permuted_scores(
  as.matrix(logCPM), 
  ## I'm a bit unclear why there are 1609 pathways with non-zero genePertScore
  ## but when we get the output here, there's 1649. 
  ## Wouldn't we drop the all-zero ones? Or didn't we do that above?
  ## Would my suggestion in the next line be a good strategy?
...
random_pathways <- permutedScore %>%
  ## Why is this `keep()` statement here? 
  ## Didn't we claim above that these are already dropped?
  ## Wouldn't it be sensible to add an argument `drop = TRUE/FALSE` which
  ## automatically drops zero-score pathways
  keep(~all(. != 0)) %>% 
  .[sample(seq_along(.), 6)] %>%
  as.data.frame() %>%
  pivot_longer(
...
In comparison, we also performed pathway analysis on this example dataset using two non-topological-based approaches: 1) *GSEA* basing on the ranking statistics derived from the differential expression (DE) analysis and 2) the rotation gene set testing for linear models (*roast*), which does not rely on DE analysis results.
The pathway *SUMOylation of DNA damage response and repair proteins* was also defined as significantly impacted by the chemotherapy using the two non-topological-based methods(see extended data), however, the expected direction of pathway perturbation can only be obtained using *sSNAPPY*.
The existing topology-based method *SPIA*, on the other hand, only considers pathways containing differentially expressed genes, hence has a lower sensitivity in detecting pathway perturbation and was only able to capture changes in immune-related processes (see extended data).
<!-- I'm thinking the comparison to ROAST & GSEA may actually be best placed as a final "Comparison to other pathway enrichment methods" section with either a figure or tables. -->
<!-- Putting it here really disrupts the workflow and that way the comparison really gets treated separately and would be the final figure/table -->
<!-- without needing an additional github repo -->
...
plot_gs2gene(
  ...
  scale_colour_gradient2(name = "logFC", low = "navyblue", high = "red") +
...
I've tried to make this easier for people to just change the pathway name & have also used some standard Z-score cutoffs which can then be used for any pathway as they're based on N(0, 1) instead of the data.
