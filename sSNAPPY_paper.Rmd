...
We also performed pathway analysis on this example dataset using three other methods to compare their performances against *sSNAPPY*.
...
The first method applied was the existing group-level topology-based method *SPIA*[@Tarca2009].
Since *SPIA* only considers pathways containing at least one differentially expressed genes, it has a lower sensitivity and was not able to detect any significant pathway perturbation in this dataset where very few genes were differentially expressed.
Additionally, we also performed pathway analysis on this example dataset using two non-topological-based approaches: 1) *GSEA*[@Subramanian2005-lx] basing on the ranking statistics derived from the differential expression analysis and 2) the fast version of rotation gene set testing for linear models (*roast*)[@wu2010roast] *fry*, which does not rely on DE analysis results.
While there was large overlap in the significantly impacted pathways revealed by *GSEA*, *fry* and *sSNAPPY*, *sSNAPPY* identified processes that could not be detected using the other two methods.  
Moreover, although both *GSEA* and *fry* try to infer the directionality of change in pathway activity, they only base it on whether genes from a pathway are more enriched in up- or down-regulated genes, ignoring the fact that genes can have both an activator and a repressor role in biological processes.
...
*fry* identified this pathway as activated as the only expressed gene in this pathway *CLU* was significantly up-regulated among post-chemotherapy samples. 
However, the topology of this pathway indicates that *CLU* is a suppressor of the process, which *sSNAPPY* was able to account for.
Through comparing *sSNAPPY* against existing pathway analysis methods, both topological and non-topological, the strength and novelty of *sSNAPPY* were demonstrated.
