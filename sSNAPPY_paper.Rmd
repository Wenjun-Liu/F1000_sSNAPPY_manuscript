...
In comparison, we also performed pathway analysis on this example dataset using two non-topological-based approaches: 1) *GSEA*[@Subramanian2005-lx] basing on the ranking statistics derived from the differential expression (DE) analysis and 2) the rotation gene set testing for linear models (*roast*)[@wu2010roast], which does not rely on DE analysis results.
While there was large overlap in the significantly impacted pathways revealed by *GSEA*, *roast* and *sSNAPPY*, *sSNAPPY* identified processes that could not be detected using the other two methods.  
Additionally, the pathway *SUMOylation of DNA damage response and repair proteins* was defined as significantly impacted by the chemotherapy using all 3 methods, however, the expected direction of pathway perturbation can only be obtained using *sSNAPPY*.
We also applied the existing group-level topology-based method *SPIA*[@Tarca2009] to the example dataset.
Since *SPIA* only consideres pathways containing at least one differentially expressed genes, it has a lower sensitivity in detecting pathway perturbation and was only able to capture changes in immune-related processes.
Details on the implementation of the other three methods and the comparison of results are available on [10.5281/zenodo.10059952](https://zenodo.org/doi/10.5281/zenodo.10059951).
