...
The first step, performed internally by *sSNAPPY*, is to estimate sample-specific log fold-change ($\delta_{gi} = \mu_{gi} - \mu_{gi'}$) across all genes $g$ within each treated sample $i$, by subtracting expression estimates for the corresponding baseline samples $\mu_{gi'}$ from those in the treated groups.
*sSNAPPY* supports any number of treatment levels and the numbers of samples in each treatment group do not have to be identical. 
...
Subsequently, the median and median absolute deviation (MAD) of the permuted perturbation scores will be calculated and used to normalise the raw perturbation scores to robust $Z$-scores ($Z_{ip} = \frac{S_{ip} - median_{n \in N, i \in I}({S_{i^np}})}{MAD_{n \in N, i \in I}({S_{i^np}})}$) and obtain associated two-sided *p*-values. 
...
The output of `weight_ss_fc()` is a `list` where one element is a matrix of weighted single-sample logFCs ($\delta_{gi}^*$), with rows corresponding to genes and columns to samples, and the other element is the vector of gene-wise weights ($w_g$) used to calculate the weighted logFC ($\delta_{gi}^*$), as described above.
...
The string `ENTREZID:` was also added to all row names of the $\delta_{gi}^*$ matrix to be compatible with the format pathway topologies were retrieved in. 
...
In the following steps, we first calculate the gene-level contributions to each pathway ($S_{gip}$) using the function `raw_gene_pert()` and then obtain pathway-level summaries using `pathway_pert()`.
